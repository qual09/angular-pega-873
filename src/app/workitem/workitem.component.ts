...
export class WorkitemComponent implements OnInit {
  ...
  bScreenflow: boolean = false;
  bLastStep: boolean = false;
  ...
  bUseScreenFlow: boolean = false;

  // To keep track of whether the form has changed or not
  bDirtyFlag: boolean = false;
  ...
  formBtnsInfo={bScanButtons: true, aBtns: []};
  aStdBtnLabels: Object;
  formButtonActionIDs: Object = formButtonActionIDs;
  bHasActionButtons: boolean = false;

  updatedFields = new Set();
  previousState: Object = new Object();
  ...
  handleRefreshAssignmentActions(sAction: string, oAction: any) {
    ...
    switch(sAction) {
      case "addRow" :
        let postContentAdd = this.refHelper.getPostContent(this.state, this.updatedFields);
        ...
        break;
      case "removeRow" :
        let postContentRemove = this.refHelper.getPostContent(this.state, this.updatedFields);
      ...
      assignmentResponse => {
        ...
        if(this.formBtnsInfo.bScanButtons){
          ...
          btnGroups.forEach( (grp) => {
            let aSecButtons = nextAssignment?.actionButtons?.[grp];
        ...
        if (nextAssignment.actions.length > 0) {
          ...
          let steps = nextAssignment?.navigation ? nextAssignment.navigation.steps : null;
          ...
          response => {
            ...
            if (endpoints.SHOWFLATLIST) {
              this.aservice.getFieldsForAssignmentFlatList(assignmentID, action).subscribe(
                ...
                err => {
                  let snackBarRef = this.snackBar.open("Errors from assignment flatlist:" + err.message, "Ok");
                }
              );
  ...
  getTargetAndIndex(sReference: string): object {
    ...
    if(nLeftParenIndex > 0) {
      ...
      if ((nRightParenIndex + 1) == nLastDot) {
        ...
        oReturn["type"] = Number.isNaN(sIndex) ? refTypes.GROUP : refTypes.LIST;
      }
      else {
  ...
  updatePageInstructionState(sRef: string, sValue: string, sCaseID: string, sRefType: string) {
    ...
    if (sRefType == "List") {
      ...
      if (isNaN(nIndex)) {
        this.updateEmbeddedPageInstructionState(sRef, sValue, sCaseID, null);
        return;
      }
  ...
  updatePageInstructionsWithNewValues(prevStateValues, newStateValues) {
    for(let key in newStateValues) {
      if(newStateValues[key] !== prevStateValues[key]) {
        let refType = null;
        let oRefInfo: any = this.getTargetAndIndex(key);
